...
```{r}
getwd()
setwd("C:/Users/victo/OneDrive/Documentos/TNBC_research")
getwd()
```
...
```{r}
setwd("~/TNBC_research")
file_path = "./data/db_files/GSE575678/GSE75678_series_matrix.txt.gz"
gse=getGEO(filename=file_path, getGPL = T)
```
...
ref1_boxplot = data.frame(DB_1 = rowMeans(exprs(gse), na.rm = FALSE, dims = 1))

boxplot(ref1_boxplot,outline=FALSE)
```
...
pheatmap(corMatrix[0:20,0:20], annotation_col=sampleInfo[c("TNBC_status", "histological grade:ch1", "response to treatment:ch1", "breast affected:ch1")])  
```
...
write_csv(full_output, file="~/TNBC_research/code/exploratory/GSE75678_clean.csv")
```
...
write_csv(full_output, path="~/TNBC_research/code/exploratory/GSE575678_clean_with_IDs_only.csv")
```
...
ref5_boxplot = data.frame(DB_5 = rowMeans(exprs(gse), na.rm = FALSE, dims = 1))
boxplot(ref5_boxplot,outline=FALSE)
# 
...
for (i in 1:nrow(sampleInfo)){
  if (isTRUE(sampleInfo[i,"characteristics_ch1.12"]=="er status: N" & sampleInfo[i,"characteristics_ch1.14"]=="pr status: N" & sampleInfo[i,"characteristics_ch1.15"]=="her 2 status: N")){
    tnbc_col <- c(tnbc_col, TRUE)
    print(c(TRUE, i))
  }else{
    tnbc_col <- c(tnbc_col, FALSE)
...
ref2_boxplot = data.frame(DB_2 = rowMeans(exprs_data, na.rm = FALSE, dims = 1))
boxplot(ref2_boxplot,outline=FALSE)
...
Reading clinical data
```{r}
raw_brca_clin <- read.delim(file = "~/TNBC_research/data/db_files/Ref 3/Ref3_untar/brca_cptac_2020/data_clinical_sample.txt", sep = "\t", header = T, row.names = 1, as.is = T)
...
Reading expression data
```{r}
exprs_data <- read.delim(file = "~/TNBC_research/data/db_files/Ref 3/Ref3_untar/brca_cptac_2020/data_mrna_seq_fpkm.txt", sep = "\t", header = T)
...
```{r}
# Modifying expression data to be normalized with log2
exprs_data_log2 <- log2(exprs_data+1)
ref3_boxplot = data.frame(DB_3 = rowMeans(exprs_data, na.rm = FALSE, dims = 1))
boxplot(ref3_boxplot,outline=FALSE)
# 
...
Reading clinical data
```{r}
raw_brca_clin <- read.delim(file = "C:/Users/victo/OneDrive/Documentos/TNBC_research/Database exploring/DB compressed files/Ref 4/Ref4_untar/brca_mbcproject_wagle_2017/data_clinical_patient.txt", sep = "\t", header = T, row.names = 1, as.is = T)
...
ref4_boxplot = data.frame(DB_4 = rowMeans(exprs_data, na.rm = FALSE, dims = 1))
boxplot(ref4_boxplot,outline=FALSE)
# 
#boxplot(exprs_data[,0:10],outline=FALSE)
```
...
#### Boxplot
```{r}
ref6_boxplot = data.frame(DB_6 = rowMeans(t(brca_rnaseq_n), na.rm = FALSE, dims = 1))
boxplot(ref6_boxplot,outline=FALSE)
...
library(ggplot2)
# Basic histogram
ggplot(brca_clin, aes(x=patient.ethnicity)) + geom_histogram(stat = "count")
# Change the width of bins
ggplot(brca_clin, aes(x=patient.ethnicity)) + 
  geom_histogram(binwidth=1, stat = "count")
# Change colors
p<-ggplot(brca_clin, aes(x=patient.ethnicity)) + 
  geom_histogram(color=viridis(3), fill=viridis(3), stat = "count")
p
```

```{r}
#boxplot(split(ref1_boxplot, ref2_boxplot, ref3_boxplot, ref4_boxplot, ref5_boxplot, ref6_boxplot))

#library(ggplot2)
#> Warning: package 'ggplot2' was built under R version 3.5.3
#ggplot(split(ref1_boxplot, ref2_boxplot, ref3_boxplot, ref4_boxplot, ref5_boxplot, ref6_boxplot)) + geom_boxplot()

sq = seq(40759)
df <- data.frame(ref1_boxplot[sq,], ref2_boxplot[sq,], ref3_boxplot[sq,], ref4_boxplot[sq,], ref5_boxplot[sq,], ref6_boxplot[sq,])

                 #vars = rep(c("DB_1","DB_2","DB_3","DB_4","DB_5","DB_6")))
boxplot(df, las=2, names= c("DB_1","DB_2","DB_3","DB_4","DB_5","DB_6"))
