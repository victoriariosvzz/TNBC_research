...
TCGA_vec<- data.frame("TCGA_vec" = rep(1, length(row.names(TCGA_db))))
#row.names(TCGA_vec) <- row.names(TCGA_db)
TCGA_vec$id <- row.names(TCGA_db)
#rm(TCGA_db)

Ref2_vec <- data.frame("Ref2_vec" = rep(1, length(row.names(Ref2_db))))
#row.names(RefGSE575678_vec) <- row.names(RefGSE575678_db)
Ref2_vec$id <- row.names(Ref2_db)
#rm(Ref2_db)

Ref3_vec <- data.frame("Ref3_vec" = rep(1, length(row.names(Ref3_db))))
#row.names(Ref3_vec) <- row.names(Ref3_db)
Ref3_vec$id <- row.names(Ref3_db)
#rm(Ref3_db)

Ref4_vec <- data.frame("Ref4_vec" = rep(1, length(row.names(Ref4_db))))
#row.names(Ref4_vec) <- row.names(Ref4_db)
Ref4_vec$id <- row.names(Ref4_db)
#rm(Ref4_db)

RefGSE20271_vec <- data.frame("RefGSE20271_vec" = rep(1, length(row.names(RefGSE20271_db))))
#row.names(RefGSE20271_vec) <- row.names(RefGSE20271_db)
RefGSE20271_vec$id <- row.names(RefGSE20271_db)
#rm(RefGSE20271_db)

RefGSE575678_vec <- data.frame("RefGSE575678_vec" = rep(1, length(row.names(RefGSE575678_db))))
#row.names(RefGSE575678_vec) <- row.names(RefGSE575678_db)
...
pheatmap(df_all_mat, main = "Gene intersection between TNBC datasets", annotation_names_col = F, show_colnames = F)
```
...
# store rownames in new column
#sampleInfo_TNBC$row.names <- rownames(sampleInfo_TNBC)
#full_annotation <- sampleInfo_TNBC
...
cy_mat_type[c(grep("normal", geoGSE25071_clin$Tissue), (which(coadread_sample == "11A")+120))] <- "Normal"

#Plot PCA Tissue Type
par(mfrow=c(1,2))
plot(cy_mat_combat_pca$rotation[,1], cy_mat_combat_pca$rotation[,2], xlab="PCA component 1", ylab="PCA component 2", main="Combat", col=cy_mat_batch, pch=19)
legend(x=-0.036, y=0.135, legend=c("GSE25071", "GSE39084", "TCGA"), col=c(1,2,3), pch=19)
plot(cy_mat_combat_pca$rotation[,1], cy_mat_combat_pca$rotation[,2], xlab="PCA component 1", ylab="PCA component 2", main="Combat", col=ifelse(cy_mat_type == "Normal", 1, 2), pch=19)
legend("topright", legend=c("Normal","Cancer"), col=1:2, pch=19)
```
