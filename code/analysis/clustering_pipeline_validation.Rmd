...
for (feature in features_to_engineer) {
  ...
  if (feature == "histological_type") {
  ...
  } else if (feature == "days_to_last_followup" || feature == "days_to_death") {
    ...
    for (i in 1:nrow(RNA_clin_data)) {
      ...
      if (is.na(RNA_clin_data$days_to_death[i]) != T) {
        #RNA_clin_data$Surv_months[i] <-
        #  RNA_clin_data$days_to_last_followup[i] / 30
        RNA_clin_data$Surv_days[i] <-
          RNA_clin_data$days_to_death[i]
        RNA_clin_data$Surv_event[i] <- 1
      }
      if (is.na(RNA_clin_data$days_to_last_followup[i]) == T && is.na(RNA_clin_data$days_to_death[i]) == T && is.na(RNA_clin_data$vital_status[i]) != T && is.na(RNA_clin_data$survival_months[i]) != T) {
        #RNA_clin_data$Surv_months[i] <-
        #  RNA_clin_data$days_to_last_followup[i] / 30
        ...
        if (RNA_clin_data$vital_status[i] == "alive" || RNA_clin_data$vital_status[i] == "Living") {
          RNA_clin_data$Surv_event[i] <- 0
        }else if (RNA_clin_data$vital_status[i] == "dead" || RNA_clin_data$vital_status[i] == "Died of Other Causes" || RNA_clin_data$vital_status[i] == "Died of Disease") {
          RNA_clin_data$Surv_event[i] <- 1
        }
      }
    }
...
cat_vars_clin <- clinical_vars_names[-c(1,6,14,15,19,20,21)]
num_vars_clin <- clinical_vars_names[c(1,6,14,15,19,20,21)]
```
...
survival_clin_not_nulls = RNA_clin_data[is.na(RNA_clin_data$Surv_days)!=T & is.na(RNA_clin_data$Surv_event)!=T,c(21,22,23)]
...
```{r}
fit_days_summary <- summary(fit_days)
```
...
# Fit Cox Model
cox <-
  coxph(Surv(Surv_days, Surv_event) ~ consensuscluster,
...
for (i in 1:2) {
  ...
  aux_mut_test <- t(sapply(1:nrow(rna_mut), function(x) {
    ...
    if (length(temp_df) > 0 && length(unique(temp_df$mut)) == 2) {
      ...
      mut_rna_test_names <- paste(rownames(mut_matrix_rna)[i], rownames(rna_mut)[x], sep = "_")
      row_aux_test <- data.frame(aux_test, row.names = c(mut_rna_test_names))

      #c(aux_test$p.value, rna_mut[x])
      #c(aux_test$p.value, c(rna_mut[x,]))
    }
  }))
  
  
  # appending each test into a vector
  mut_rna_test_df <- rbind(mut_rna_test_df, t(aux_mut_test))
}

# we save the P values and FDR of each test
#mut_rna_test_unlisted <- unlist(mut_rna_test)
mut_rna_test <- do.call(rbind.data.frame, mut_rna_test_df$V1)
mut_rna_test <- cbind(mut_rna_test, p.adjust(mut_rna_test$aux_test, method='fdr'))
...
write.csv(
  ...
  "~\\TNBC_research\\code\\csv_and_objects\\mutations\\Mutations_merged_T_test.csv",
  row.names = FALSE
)
```
...
```{r}
contrasts_c1 <- makeContrasts(Cluster1 - (Cluster2 + Cluster3)/2, levels = design)
contrasts_c2 <-
  makeContrasts(Cluster2 - (Cluster1 + Cluster3)/2, levels = design)
contrasts_c3 <-
  makeContrasts(Cluster3 - (Cluster1 + Cluster2)/2, levels = design)

fitc1 <- contrasts.fit(fit, contrasts_c1)
fitc2 <- contrasts.fit(fit, contrasts_c2)
fitc3 <- contrasts.fit(fit, contrasts_c3)
```
...
```{r}
full_resultsc1 <- topTable(fitc1, number = Inf)
full_resultsc1 <- tibble::rownames_to_column(full_resultsc1, "ID")

full_resultsc2 <- topTable(fitc2, number = Inf)
full_resultsc2 <- tibble::rownames_to_column(full_resultsc2, "ID")

full_resultsc3 <- topTable(fitc3, number = Inf)
full_resultsc3 <- tibble::rownames_to_column(full_resultsc3, "ID")
```
...
```{r}
sign_genes_c1 <- filter(full_resultsc1, adj.P.Val < p_cutoff)
sign_genes_c2 <- filter(full_resultsc2, adj.P.Val < p_cutoff)
sign_genes_c3 <- filter(full_resultsc3, adj.P.Val < p_cutoff)
```
...
library(readr)
full_resultsc1 %>%
  write_csv(file="complete_DEGs_results_c1_18_oct_22.csv")

full_resultsc2 %>%
  write_csv(file="complete_DEGs_results_c2_18_oct_22.csv")

full_resultsc3 %>%
  write_csv(file="complete_DEGs_results_c3_18_oct_22.csv")
```


Comparing the diferentially expressed genes between all the clusters
```{r}
significant_genes <- filter(full_resultsc1, p.adjust(P.Value, method="fdr") < p_cutoff)$ID
...
##
ids_of_interest1 <- mutate(full_resultsc1, Rank = 1:n()) %>%
  filter(Rank < topN) %>%
  pull(ID)
ids_of_interest2 <- mutate(full_resultsc2, Rank = 1:n()) %>%
  filter(Rank < topN) %>%
  pull(ID)
ids_of_interest3 <- mutate(full_resultsc3, Rank = 1:n()) %>%
  filter(Rank < topN) %>%
  pull(ID)
```
...
```{r}
gene_names1 <- mutate(full_resultsc1, Rank = 1:n()) %>% 
  filter(Rank < topN) %>% 
  pull(ID) 
gene_names2 <- mutate(full_resultsc2, Rank = 1:n()) %>% 
  filter(Rank < topN) %>% 
  pull(ID) 
gene_names3 <- mutate(full_resultsc3, Rank = 1:n()) %>% 
  filter(Rank < topN) %>% 
  pull(ID) 
```
...
gene_matrix1 <- t(RNA_seq_exprs)[ids_of_interest1,]
gene_matrix2 <- t(RNA_seq_exprs)[ids_of_interest2,]
gene_matrix3 <- t(RNA_seq_exprs)[ids_of_interest3,]
```

We now make the heatmap. A default colour scheme is used, but can be changed via the arguments. 
```{r}
pheatmap(gene_matrix1[,1:20],
     labels_row = gene_names1,
     scale="row")
```
```{r}
pheatmap(gene_matrix2[,1:20],
     labels_row = gene_names2,
     scale="row")
```
```{r}
pheatmap(gene_matrix3[,1:20],
     labels_row = gene_names3,
     scale="row")
```
## PATHWAYS
