...
train_mad <- read.csv("ComBat/ComBat_mad.csv", row.names = "row.names")
...
# Change the working directory to the folder where the the file lives
setwd(
  "C:\\Users\\victo\\OneDrive\\Documentos\\TNBC_research\\Clinical_data_comparison"
)

# Read the csv file with the clinical information
combo_combat_mad <-
  read.csv("clinical_data_combat_mad.csv", row.names = "row.names")
...
x_train <- prcomp(t(train_mad), scale = TRUE)
```
...
RNA_seq_merged_allgenes <-
  as.data.frame(t(rbind(
    train, t(RNA_seq_cluster_annotation)
  )))
...
```{r}
dat.gct <- as.data.frame(read.delim(file="C:\\Users\\victo\\OneDrive\\Documentos\\TNBC_research\\Clustering\\neu_nmfconsensus.k.2.gct"))
