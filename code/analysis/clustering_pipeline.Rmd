...
Before proceeding with the numerical analysis, we remove the outlier
sample (BA00439)
```{r}
...
Pass the transposed expression dataframe to the prcomp() function, so
each row represents a sample and each column a gene.
```{r}
...
Visual verification of the differences across different pairs of
components. This helps identifying differential descriptive Principal
Components across different variables, if there is no clear difference,
only the first 2 PCs are kept to continue the global analysis.
...
There isn't an apparent difference in the data explained by the first 6
components with the highest explained variance, so, we proceed with the
analysis with a visualization focused on the first two PCs.
...
**K-means clustering** We proceed to apply a K-means clustering
procedure on the PCA selected components.
```{r}
set.seed(100)
...
common_cols_train_val <- intersect(colnames(RNA_seq_merged_allgenes), colnames(RNA_seq_merged_allgenes_val))
temp_train <- RNA_seq_merged_allgenes[,names(RNA_seq_merged_allgenes) %in% common_cols_train_val]
temp_val <- RNA_seq_merged_allgenes_val[,names(RNA_seq_merged_allgenes_val) %in% common_cols_train_val]

# Now, we remove the clinical information to generate the classification model based on gene expression
temp_train <- temp_train[,!names(temp_train) %in% c(clinical_vars_names, clinical_vars_names_val, "Surv_months")]
temp_val <- temp_val[,!names(temp_val) %in% c(clinical_vars_names, clinical_vars_names_val, "Surv_months")]

# Split data into partitions
set.seed(100)
temp_split <- partition(temp_train$consensuscluster, p = c(train = 0.8, test = 0.2))
str(temp_split)
...
train_classif_orig <- temp_train[temp_split$train, ]
test_classif_orig <- temp_train[temp_split$test, ]

# We verify the quantity of samples per cluster of each split
table(train_classif_orig$consensuscluster)
table(test_classif_orig$consensuscluster)
```
...
test_classif$V2455 <- as.numeric(test_classif$V2455)

m1 <- m2 <- 200
dat.m1 <- mhingebst(x=train_classif, y=train_classif$cluster, ctrl = bst_control(mstop=m1), family = "hinge", learner = c("ls"))

risk.te1 <- predict(dat.m1, newdata=test_classif, newy=test_classif$V2455, mstop=m1, type="error")
...
risk.te1 <- predict(dat.m2, newdata=test_classif, newy=test_classif$V2455, mstop=m2, type="error")
plot(risk.te1, type="l", xlab="Iteration", ylab="Test Error")
```
Plot the evolution of the number of genes selected versus the iteration
counter, as the multi-class twin HingeBoost algorithm proceeds while working
on the training set.
```{r}
...
```{r}
# plot
gp = xgb.ggplot.importance(importance_matrix[1:30,], n_clusters = 3)
print(gp) 
```
...
```{r}
# Predict hold-out test set
val_pred <- predict(bst_model, newdata = val_matrix)
val_prediction <- matrix(val_pred, nrow = numberOfClasses,
                          ncol=length(val_pred)/numberOfClasses) %>%
  t() %>%
  data.frame() %>%
  mutate(label = val_label + 1,
         max_prob = max.col(., "last"))
# confusion matrix of test set
confusionMatrix(factor(val_prediction$max_prob),
                factor(as.integer(c(as.vector(t(val_prediction$label-1))[-146], 3))),
                mode = "everything")
```
...
Plot of the clinical variables



### Option 3: MLSeq classification models
MLSeq is a comprehensive package for application of machine learning algorithms in classification of RNA-Seq data. Researchers have appealed to MLSeq for various
purposes, which include prediction of disease outcomes, identification of best subset of features (genes, transcripts, other isoforms), and sorting the features based on their predictive importance. Transformation approaches can be used to bring discrete RNA-seq data hierarchically closer to microarrays and conduct microarray-based classification algorithms. Currently, MLSeq package contains 90+ microarray-based classifiers including the recently developed voom-based discriminant analysis classifiers.
...
### Correlation between clinical information of the validation set and 
```{r}
...
```{r}
clinical_plots(val_data_RNA_clin, clinical_vars_names_val, "counts", 3)
```
...
**Numeric variables** Running the statistical tests for the numerical
variables
...
First we get the genes set corresponding to the "homo sapiens" specie
(human).
...
Filter the main genes dataframe to include only the genes mainly
...
-   Calcular score para cada muestra, sin importar si son significativos
    o no
