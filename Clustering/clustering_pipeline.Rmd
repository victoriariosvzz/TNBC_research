...
```
## Use ComBat results instead
```{r}

setwd("C:\\Users\\victo\\OneDrive\\Documentos\\TNBC_research\\Database exploring")
train <- read.csv("ComBat/ComBat_train.csv", row.names = "row.names")
...
x_test <- prcomp(test, scale = FALSE)
```
...
```{r}
row.names(x_train[["x"]]) <- short_IDs_train
...
```{r}
# for vignette
res_train <- M3C(t(train), removeplots = TRUE, iters=3, objective='PAC', fsize=8, lthick=1, dotsize=1.25)
...
```{r}
res_entropy_train <- M3C(t(train), method = 2,fsize=8, lthick=1, dotsize=1.25, removeplots = TRUE)
