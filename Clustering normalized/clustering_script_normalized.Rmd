...
```{r}
brca_clin <- read.delim(file = "C:/Users/victo/OneDrive/Documentos/TNBC_research/BRCA.clin.merged.txt", sep = "\t", header = F, row.names = 1, as.is = T)
```
...
```{r}
f <- "C:/Users/victo/OneDrive/Documentos/R/TNBC research/TNBC_research/BRCA.rnaseqv2__illuminahiseq_rnaseqv2__unc_edu__Level_3__RSEM_genes_normalized__data.data.txt"
...
brca_rnaseq_n <- read.csv("ComBat/ComBat_db.csv", row.names = "row.names")
```

```{r}
#MAD to filter genes and PCA
cy_mat_combat_mad <- apply(brca_rnaseq_n, 1, mad)
brca_rnaseq_n <- brca_rnaseq_n[which(cy_mat_combat_mad > 1),]
```
